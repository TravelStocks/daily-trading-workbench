...
export default function RecapDigest({day}:{day?:Day}){
 ...
 <article className="digest-section"><h3><span>03</span>题材梯队</h3>{data.themes.length?<div className="digest-themes">{data.themes.map(t=><div className="digest-theme" key={t.name}><div className="digest-theme-heading"><strong>{t.name}</strong><span>{t.role}</span></div><ul>{t.ladder.map(s=><li key={s.name}><b>{s.name}</b><span>{s.board}</span><small>{s.role}</small></li>)}</ul>{t.outlook&&<p>次日：{t.outlook}</p>}</div>)}</div>:<Missing/>}</article>
