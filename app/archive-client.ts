...
async function fetchArchive(){
 let data:Archive;
 try{const remote=await getJson('assets/cycle-history.json') as Archive;if(remote.version!==1||!Array.isArray(remote.days)||!Array.isArray(remote.points))throw new Error('Invalid archive');data={...remote,source:'live'}}catch{const response=await fetch(import.meta.env.BASE_URL+'data/history.json');if(!response.ok)throw new Error('复盘资料暂时无法读取，请重试。');data={...(await response.json() as Archive),source:'snapshot'}}
...
export async function loadArchive(force=false){if(!force&&cached&&Date.now()-stamp<60000)return cached;if(inflight)return inflight;inflight=fetchArchive().then(d=>{cached=d;stamp=Date.now();return d}).finally(()=>{inflight=null});return inflight}
